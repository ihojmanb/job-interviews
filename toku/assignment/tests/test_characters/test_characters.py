...
class TestCharacters:
    def test_build_character_success(self, random_id, superhero_consumer, character_factory):
        character_data = superhero_consumer.get_character(random_id)
        character = character_factory.build_character(character_data)
        assert character
        assert isinstance(character, Character)

    def test_build_character_fail(self, superhero_consumer, character_factory):
        character_data = superhero_consumer.get_character(0)
        character = character_factory.build_character(character_data)
        assert not isinstance(character, Character)
    
    def test_assign_stats_stamina_success(self, random_id, superhero_consumer, character_factory):
        character_data = superhero_consumer.get_character(random_id)
        character = character_factory.build_character(character_data)
        for attribute in attributes:
            assert hasattr(character, f"AS_{attribute}")

    def test_assign_stats_stamina_fail(self, superhero_consumer, character_factory):
        character_data = superhero_consumer.get_character(0)
        character = character_factory.build_character(character_data)
        for attribute in attributes:
            assert not hasattr(character, f"AS_{attribute}")

    def test_assign_global_stamina_success(self, random_id, superhero_consumer, character_factory):
        character_data = superhero_consumer.get_character(random_id)
        character = character_factory.build_character(character_data)
        assert hasattr(character, "actual_stamina")

    def test_assign_global_stamina_fail(self, superhero_consumer, character_factory):
        character_data = superhero_consumer.get_character(0)
        character = character_factory.build_character(character_data)
    ...
    def test_correct_health_power_success(self, good_character, good_character_stats):
        ...
        assert hasattr(character, "health_points")
        actual_health_points = character.health_points
